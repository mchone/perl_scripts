...
while (<GFF>){
    ...
    next if /^#/;
    my @arr=split/\t/;
    my @tmp =split /;/, $arr[8];#gene loc, first part of last column of gff3 files.
    for my $chr(keys %vcf){
        if ($chr eq $arr[0]){
            for my $pos(sort keys%{$vcf{$chr}}){
                if ($pos>=$arr[3] && $pos <=$arr[4]){
                printf OU "$tmp[0]\t%.2f\t$vcf{$chr}{$pos}\n",$per{$chr}{$pos};#print first part of gene loc; var percentage 2 float;  
		}
	     next;
            }
        }
